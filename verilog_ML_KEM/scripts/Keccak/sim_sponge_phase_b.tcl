# ==============================================================================
# R-new-A Phase B: tb_keccak_sponge_top lane-absorb regression
# ==============================================================================
# Verifies that the new lane-mode absorb path produces byte-for-byte identical
# Keccak output as the byte-mode path. Re-runs all 6 legacy SHAKE/SHA3 KAT
# testcases (still byte-mode) and adds testcase 7: 200-byte SHAKE128 absorbed
# via the lane interface, expected output = exp_multi_absorb.
#
# CLEANUP (if locked): taskkill /F /IM xsimk.exe /T & taskkill /F /IM xsim.exe /T & taskkill /F /IM vivado.exe /T
#
# EXECUTION (CMD):
# cd /d D:\HCMUT\Year_4\252\CA\Source\DACN && "C:\Xilinx\2025.1\Vivado\bin\vivado.bat" -mode batch -source verilog_ML_KEM/scripts/Keccak/sim_sponge_phase_b.tcl -tclargs D:/HCMUT/Year_4/252/CA/Vivado/verilog_ML_KEM/verilog_ML_KEM.xpr
# ==============================================================================

if {[llength $argv] < 1} {
    puts "ERROR: Missing project path argument"
    puts "Usage: vivado -mode batch -source sim_sponge_phase_b.tcl -tclargs <xpr_path>"
    exit 1
}
...
set run_dir [file join $base_out_dir "sim_sponge_phase_b_$ts"]
...
# in effect). See sim_core_phase_a.tcl for context.
...
file copy -force $xsim_log [file join $run_dir "phase_b_simulate.log"]

set fp [open [file join $run_dir "phase_b_simulate.log"] r]
set log_txt [read $fp]
close $fp

# Phase B signoff requires ALL 7 PASS markers (6 legacy KAT + 1 lane-mode KAT)
set must_have [list \
    "SHAKE128 PASSED" \
    "SHAKE256 PASSED" \
    "SHA3-256 PASSED" \
    ...
    "ALL 7 TESTCASES PASSED FLAWLESSLY"]
...
foreach p $must_have {
    if {[string first $p $log_txt] < 0} {
        puts "ERROR: Phase B TB missing PASS marker: $p"
        close_project
        exit 3
    }
}
...
foreach p $must_not_have {
    if {[string first $p $log_txt] >= 0} {
        puts "ERROR: Phase B TB found failure marker: $p"
        close_project
        exit 4
    }
}

puts "PASS: R-new-A Phase B regression gate passed"
