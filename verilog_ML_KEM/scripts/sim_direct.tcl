...
if {[llength $argv] < 1} {
    ...
    puts "Usage: vivado -mode batch -source sim_direct.tcl -tclargs <xpr_path> [jobs]"
    exit 1
}
...
set run_dir [file join $base_out_dir "sim_direct_$ts"]
...
set_property top tb_ml_kem_keygen [get_filesets sim_1]
...
file copy -force $xsim_log [file join $run_dir "direct_tb_simulate.log"]

set fp [open [file join $run_dir "direct_tb_simulate.log"] r]
set log_txt [read $fp]
close $fp

set must_have [list "ALL KAT PASSED: 100 vectors" "Boundary all-zero seeds: PASSED" "Boundary all-FF seeds: PASSED"]
set must_not_have [list "PK Mismatch" "SK Mismatch" "HPK stream mismatch" "TIMEOUT" "\$stop called"]
...
puts "PASS: direct TB regression gate passed"
