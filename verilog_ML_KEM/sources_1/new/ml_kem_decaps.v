        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_HASH_G_INIT: begin
                    ...
                    // Explicit byte mode for G. Without this, a residual 1
                    // carried over from the prior call's S_HASH_J_INIT
                    // (only ever cleared on hard reset) would latch the
                    // sponge into ABSORB_LANE while the G FSM here still
                    // drives the byte k_din path → permanent deadlock on
                    // every 2nd+ Decaps. Same pattern as ml_kem_encaps.
                    absorb_lane_mode_reg <= 1'b0;
                    var_k       <= 12'd0;
                    k_din       <= m_prime[5'd0];
                    k_din_valid <= 1'b1;
